...
public class ThumbnailAdapter1 extends RecyclerView.Adapter<ThumbnailAdapter1.ThumbnailHolder> {

    private static final int LAYOUT_VIEW_TYPE_ADD_PIC       = 1;
    private static final int LAYOUT_VIEW_TYPE_THUMBNAIL_PIC = 2;

    private final boolean isStartWithAddPic = false;
    ...
    public ThumbnailAdapter1(BaseFragment mFragment, List<ThumbnailBean> thumbnailBeanList) {
    ...
    public ThumbnailAdapter1(BaseFragment mFragment, List<ThumbnailBean> thumbnailBeanList, int maxCount) {
        this.thumbnailBeanList = thumbnailBeanList;
        this.mActivity = mFragment.getActivity();
        this.mFragment = mFragment;
    ...
    public ThumbnailHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == LAYOUT_VIEW_TYPE_ADD_PIC) {
            View view = LayoutInflater.from(mActivity).inflate(R.layout.item_pic_thumbnail_add1, null);
            return new ThumbnailHolder(view);
        } else if (viewType == LAYOUT_VIEW_TYPE_THUMBNAIL_PIC) {
            View view = LayoutInflater.from(mActivity).inflate(R.layout.item_pic_thumbnail1, null);
            return new ThumbnailHolder(view);
        }
        return null;
    }
