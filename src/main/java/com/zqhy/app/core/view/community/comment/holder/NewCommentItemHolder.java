 ...
 */
public class NewCommentItemHolder extends AbsItemHolder<CommentInfoVo.DataBean, NewCommentItemHolder.ViewHolder> {

    private float density;

    public NewCommentItemHolder(Context context) {
        super(context);
    ...
    protected void onBindViewHolder(@NonNull ViewHolder holder, @NonNull CommentInfoVo.DataBean item) {
        ...
        if (item.getReply_list() != null) {
            ...
            if (replyListSize > 1) {
                replyBean.setCid(-1);
                replyBeanList.clear();
                replyBeanList.add(item.getReply_list().get(0));
    ...
    public void setReplyData(TextView mTv, CommentInfoVo.ReplyInfoVo replyInfoBean, int position) {
        mTv.setTextSize(14);
        mTv.setTextColor(Color.parseColor("#333333"));
        mTv.setLineSpacing(0, 1.1f);
        AbsoluteSizeSpan absoluteSizeSpan = new AbsoluteSizeSpan((int) (14 * density));
        if (replyInfoBean.getCid() == -1) {
            ...
            int nicknameColor = Color.parseColor("#333333");
...
//            ((GameDetailInfoFragment)_mFragment).showCommentTipsDialog();
        }
    }
