...
public class EmptyItemHolder extends AbsItemHolder<EmptyDataVo, EmptyItemHolder.ViewHolder> {
    ...
    protected void onBindViewHolder(@NonNull ViewHolder holder, @NonNull EmptyDataVo item) {
        if (TextUtils.isEmpty(item.getEmptyWord())) {
            ...
            try {
                ...
                holder.mTvErrorDesc.setTextColor(ContextCompat.getColor(mContext, item.getResEmptyWordColor()));
            } catch (Exception e) {
                e.printStackTrace();
                holder.mTvErrorDesc.setVisibility(View.GONE);
            }
    ...
    @Override
    public void onViewAttachedToWindow(@NonNull EmptyItemHolder.ViewHolder holder) {
