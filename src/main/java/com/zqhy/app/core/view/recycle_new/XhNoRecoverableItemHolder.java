 ...
 * @description
 */
public class XhNoRecoverableItemHolder extends BaseItemHolder<XhGameNewRecycleListVo.DataBean, XhNoRecoverableItemHolder.ViewHolder> {

    public XhNoRecoverableItemHolder(Context context) {
        super(context);
    }
    ...
    public int getLayoutResId() {
        return R.layout.item_xh_new_layout_2;
    }
    ...
    protected void onBindViewHolder(@NonNull ViewHolder holder, @NonNull XhGameNewRecycleListVo.DataBean item) {
        ...
        holder.mTvAction.setText(item.getRemark() + " >");
        holder.mTvAction.setOnClickListener(view -> {
            if (_mFragment != null){
                ((XhUnRecoverableListFragment)_mFragment).showTipDialog();
            }
        });
    }
