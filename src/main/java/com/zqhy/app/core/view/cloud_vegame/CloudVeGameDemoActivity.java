...
public class CloudVeGameDemoActivity{}/* extends BaseActivity<CloudViewModel> implements IStreamListener, IGamePlayerListener, VeGameFloatManager.OnClick {
    ...
    public static void newInstance(Activity activity, String uid, String ts_gameId, String gameId, String icon, String gameName, String otherName, VeTokenVo veTokenVo) {
        Intent intent = new Intent(activity, CloudVeGameDemoActivity.class);
        ...
        intent.putExtra("otherName", otherName);
        intent.putExtra("veTokenVo",veTokenVo);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            veTokenVo = (VeTokenVo)getIntent().getSerializableExtra("veTokenVo");
        }catch (Exception e){
    ...
    private void initGamePlayConfig(VeTokenVo bean) {
        *//**
        ...
        secondsUntilExpiration(secondsUntilExpire);
...
//            get_extra = userInfoBean.getUid() + "_" + userInfoBean.getToken() + "_ds";//折上折平台需要加
    ...
    private void secondsUntilExpiration(long expirationTime) {
        countDownTimer = new CountDownTimer(expirationTime*1000, 1000) { // 总时间10000毫秒，每1000毫秒更新一次

            @Override
            public void onTick(long millisUntilFinished) {
                if (tv_time_over!=null){
                    tv_time_over.setVisibility(View.VISIBLE);

                    long hours = millisUntilFinished/1000 / 3600;
                    long remainder = millisUntilFinished/1000 % 3600;
                    long minutes = remainder / 60;
                    long seconds = remainder % 60;

                    // 使用SimpleDateFormat或更现代的DateTimeFormatter进行格式化
                    // 但注意，这些主要用于格式化日期和时间对象，不是直接从整数秒计算并格式化
                    // 因此，我们直接使用String.format()
                    String formattedTime = String.format("%02d:%02d:%02d", hours, minutes, seconds);
                    tv_time_over.setText("试玩时间:"+formattedTime);
                }
            }
            ...
            public void onFinish() {
                // 倒计时结束调用
                // 例如：Toast.makeText(MainActivity.this, "倒计时结束", Toast.LENGTH_SHORT).show();
                if (tv_time_over!=null){
                    tv_time_over.setText("试玩时间结束");
                }
            }
        }.start(); // 调用start()开始倒计时
...
//    }
            ...
            mMessageChannel.setMessageListener(new IMessageChannel.IMessageReceiver() {
                *//**
                ...
                public void onReceiveMessage(IMessageChannel.IChannelMessage iChannelMessage) {
                    ...
                    if (isNewMessage){
                        //如果本地没有此支付订单则网页打开
                        BrowserCloudPayActivity.newInstance(CloudVeGameDemoActivity.this, payload, true, true);
    ...
    public void onConfigurationChanged(@NonNull Configuration newConfig) {
        ...
        tv_time_over = contentView.findViewById(R.id.tv_time_over);
    ...
    private void bindView() {
        ...
        timer.schedule(new TimerTask() {
            ...
            public void run() {
                if (progress < 80) {
                    progress = progress + 2;
                    CloudVeGameDemoActivity.this.runOnUiThread(() -> {
...
//
    ...
    *//**
    ...
    public void onPlaySuccess(String roundId, int clarityId, Map<String, String> extraMap, String gameId, String reservedId) {
        ...
        mViewModel.getVolcengineDemoStart(params, new OnNetWorkListener() {
            @Override
            public void onBefore() {
            ...
            public void onSuccess(BaseVo data) {
                if (data != null && "ok".equals(data.getState())) {
                    ...
                    tv_bar.setText("开启云试玩失败,请退出后重试! msg:" + data.getMsg());
                    ll_content_layout.setVisibility(View.VISIBLE);
                    VeGameEngine.getInstance().stop();
                }
            }
                    ...
                    if (success) {
                        ...
                        if (tv_hd != null) {
                            switch (hd_type) {
                                case 2:
                                    tv_hd.setText("流畅");
                                    Toaster.show(CloudVeGameDemoActivity.this, "切换至流畅");
                                    break;
                                case 14:
                                    tv_hd.setText("高清");
                                    Toaster.show(CloudVeGameDemoActivity.this, "切换至高清");
                                    break;
                                case 16:
                                    tv_hd.setText("超清");
                                    Toaster.show(CloudVeGameDemoActivity.this, "切换至超清");
                                    break;
                                default:
                                    tv_hd.setText("标清");
                                    Toaster.show(CloudVeGameDemoActivity.this, "切换至标清");
                                    break;
                            }
                        }
                    } else {
                        ToastT.error(CloudVeGameDemoActivity.this, "清晰度切换失败");
                    }
                }
    ...
    public void onPodExit(int reasonCode, String reasonMessage) {
        ...
        try {
            ...
            if (reasonCode == 40004) {
                ...
                ll_content_layout.setVisibility(View.VISIBLE);
                tv_bar.setText("试玩时间到,感谢您的试玩!可下载此游戏继续游玩~");
                timeOver();
            } else {
                if (timer != null) {
                    timer.cancel();
