...
public class NewProvinceItemHolder extends AbsItemHolder<SuperVipMemberInfoVo.DataBean.CardType, NewProvinceItemHolder.ViewHolder> {
    private boolean isDialog;
    public NewProvinceItemHolder(Context context, boolean isDialog) {
        super(context);
    ...
    protected void onBindViewHolder(@NonNull ViewHolder holder, @NonNull SuperVipMemberInfoVo.DataBean.CardType item) {
        ...
        holder.mTvTips1.setText("立返" + item.getGive() + "元福利币");
        holder.mTvTips2.setText("每天再领5元福利币");

        SpannableString spannableString = new SpannableString("立返" + item.getGive() + "元福利币");
        spannableString.setSpan(new ForegroundColorSpan(Color.parseColor("#FF2D3F")), 2, 2 + String.valueOf(item.getGive()).length(), Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
        holder.mTvTips1.setText(spannableString);

        SpannableString spannableString1 = new SpannableString("每天再领5元福利币");
        spannableString1.setSpan(new ForegroundColorSpan(Color.parseColor("#FF2D3F")), 4, 5, Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
        holder.mTvTips2.setText(spannableString1);

        holder.mTvVipDays.setText(item.getName() + item.getDay() + "天");

        if (item.isSelected()){
            holder.mLlInnerBg.setBackgroundResource(R.mipmap.ic_province_card_option_selected);
            holder.mTvVipDays.setBackgroundResource(R.drawable.shape_fe6631_ef0f16_other_radius);
            holder.mTvPrice.setTextColor(Color.parseColor("#406AF6"));
            holder.mTvPriceType.setTextColor(Color.parseColor("#406AF6"));
