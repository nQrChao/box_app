...
public class GameDetailActivityItemHolder extends AbsItemHolder<GameActivityVo.ItemBean, GameDetailActivityItemHolder.ViewHolder> {
    ...
    protected void onBindViewHolder(@NonNull ViewHolder holder, @NonNull GameActivityVo.ItemBean item) {
        if (holder.getLayoutPosition() != listSize - 1){
            RelativeLayout.LayoutParams layoutParams = (RelativeLayout.LayoutParams) holder.mLlRootView.getLayoutParams();
            layoutParams.setMargins(ScreenUtil.dp2px(mContext, 15), 0, ScreenUtil.dp2px(mContext, 15), ScreenUtil.dp2px(mContext, 10));
            holder.mLlRootView.setLayoutParams(layoutParams);
        }else {
            RelativeLayout.LayoutParams layoutParams = (RelativeLayout.LayoutParams) holder.mLlRootView.getLayoutParams();
            layoutParams.setMargins(ScreenUtil.dp2px(mContext, 15), 0, ScreenUtil.dp2px(mContext, 15), 0);
            holder.mLlRootView.setLayoutParams(layoutParams);
        }
        ...
        if (item.getType() == 1) {
            ...
            if (newslistBean != null) {
                ...
                if (!"0".equals(newslistBean.getBegintime()) && !"0".equals(newslistBean.getEndtime())){
                    holder.mTvActivityContent.setText("限时活动：" + CommonUtils.timeStamp2Date(Long.parseLong(newslistBean.getBegintime() + "000"), "MM月dd日") + "-" + CommonUtils.timeStamp2Date(Long.parseLong(newslistBean.getEndtime() + "000"), "MM月dd日"));
                }else if (!"0".equals(newslistBean.getBegintime()) && "0".equals(newslistBean.getEndtime())){
                    holder.mTvActivityContent.setText("长期活动：" + CommonUtils.timeStamp2Date(Long.parseLong(newslistBean.getBegintime() + "000"), "MM月dd日") + "起");
                }else if ("0".equals(newslistBean.getBegintime()) && !"0".equals(newslistBean.getEndtime())){
                    holder.mTvActivityContent.setText("限时活动：截至" + CommonUtils.timeStamp2Date(Long.parseLong(newslistBean.getEndtime() + "000"), "MM月dd日"));
                }else {
                    holder.mTvActivityContent.setText("长期活动：进行中");
                }
                holder.mTvActivityTag.setVisibility(newslistBean.getIs_newest() == 1 ? View.VISIBLE : View.GONE);
                if (newslistBean.getRebate_apply_type() == 0){
                    holder.mTvActivityAction.setText("查看");
                    holder.mTvActivityAction.setTextColor(Color.parseColor("#FFFFFF"));
                    holder.mTvActivityAction.setBackgroundResource(R.drawable.shape_55c0fe_5571fe_big_radius);
                ...
                if ("strategy".equals(newslistBean.getNews_status())){
                    holder.mLlActiviContent.setVisibility(View.GONE);
                }
            }
    ...
    public class ViewHolder extends AbsHolder {
        ...
        private LinearLayout mLlActiviContent;
        ...
        public ViewHolder(View view) {
            ...
            mLlActiviContent = view.findViewById(R.id.ll_activi_content);
        }
    }
}
