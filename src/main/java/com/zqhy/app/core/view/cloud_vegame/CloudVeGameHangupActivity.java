...
public class CloudVeGameHangupActivity{}/* extends BaseActivity<CloudViewModel> implements IStreamListener, IGamePlayerListener, VeGameFloatManager.OnClick {
    private final String TAG = "VeGameEngineActivity";
    private TextView network_state;
    private TextView tv_hd;

    public static void newInstance(Activity activity, String gid, String ts_gameId, String gameId, String icon, String gameName, String otherName, int queuePriority) {
        Intent intent = new Intent(activity, CloudVeGameHangupActivity.class);
        intent.putExtra("gid", gid);
        intent.putExtra("ts_gameId", ts_gameId);//本平台游戏id
        intent.putExtra("gameId", gameId);//火云游戏id
        intent.putExtra("icon", icon);
        intent.putExtra("gameName", gameName);
        intent.putExtra("otherName", otherName);
        intent.putExtra("reservedId", "");
        intent.putExtra("queuePriority", queuePriority);
        activity.startActivity(intent);
    }

    public static void newInstance(Activity activity, String gid, String ts_gameId, String gameId, String icon, String gameName, String otherName, String reservedId, int queuePriority) {
        Intent intent = new Intent(activity, CloudVeGameHangupActivity.class);
        intent.putExtra("gid", gid);
        intent.putExtra("ts_gameId", ts_gameId);//本平台游戏id
        intent.putExtra("gameId", gameId);//火云游戏id
    ...
    protected String gid;//用户购买的设备号id
    protected String reservedId = "";//已开始挂机的设备资源id
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        reservedId = getIntent().getStringExtra("reservedId");
        queuePriority = getIntent().getIntExtra("queuePriority", 99);
    ...
    private void initData() {
        mViewModel.getToken(gid, new OnNetWorkListener<VeTokenVo>() {
            @Override
    ...
    private StreamProfileManager mClarityService;//视频质量
...
//            get_extra = userInfoBean.getUid() + "_" + userInfoBean.getToken() + "_ds";//折上折平台需要加
        ...
        Map<String, String> debug_info_inner = new HashMap<>();
        debug_info_inner.put("ak",ak);
        debug_info_inner.put("sk",sk);
        debug_info_inner.put("bucket","batu_game");
        debug_info_inner.put("endpoint","tos-cn-beijing.volces.com");
        debug_info_inner.put("region","cn-beijing");
        debug_info_inner.put("ak",ak);
        String config = map2Json(debug_info_inner);

        Map<Object, Object> debug_info_out = new HashMap<>();
        debug_info_out.put("config",debug_info_inner);
        debug_info_out.put("schema","btd_tos");
        debug_info_out.put("key","test/batu_game_"+ts_gameId+".zip");

        Gson gson = new Gson();
        String json = gson.toJson(debug_info_out);
        extraMap.put("debug_info",json);
        Log.d(TAG, " debug_info----json: " + json);
        Log.d(TAG, "roundId: " + roundId);
        Log.d(TAG, "userId: " + userId);
        Log.d(TAG, "get_extra: " + get_extra);
        Log.d(TAG, "gameId: " + gameId);
        Log.d(TAG, "reservedId: " + reservedId);
        extraMap.put("get_extra", get_extra);
        extraMap.put("enable_browser", "true");//在云端游戏中点击链接打开游览器的功能：
//        if (Integer.parseInt(gameId) % 2 == 0) {
//            gameId = "7390256447029861174";//横屏
//        } else {
//            gameId = "7391772541209041701";//竖屏
//        }
//        gameId = "7390256447029861174";//横屏
//        gameId= "7387343826937043751";//竖屏
...
//                .queuePriority(queuePriority)
        ...
        if (!reservedId.isEmpty()) {
            mBuilder.reservedId(reservedId);//重连到设备,无需排队(开启排队后,无法进入云机)
        } else {
            mBuilder.queuePriority(queuePriority);//首次挂机需,开启排队
        }
            ...
            mMessageChannel.setMessageListener(new IMessageChannel.IMessageReceiver() {
                *//**
                ...
                public void onReceiveMessage(IMessageChannel.IChannelMessage iChannelMessage) {
                    ...
                    if (isNewMessage){
                        //如果本地没有此支付订单则网页打开
                        BrowserCloudPayActivity.newInstance(CloudVeGameHangupActivity.this, payload, true, true);
    ...
    private void bindView() {
        ...
        timer.schedule(new TimerTask() {
            ...
            public void run() {
                if (progress < 80) {
                    progress = progress + 2;
                    CloudVeGameHangupActivity.this.runOnUiThread(() -> {
...
//
    ...
    *//**
    ...
    public void onPlaySuccess(String roundId, int clarityId, Map<String, String> extraMap, String gameId, String reservedId) {
        ...
        timer = null;
        if (this.reservedId.isEmpty()) {
            //开始挂机
            Map<String, String> params = new TreeMap<>();
            params.put("gid", gid);
            params.put("gameid", this.ts_gameId);//本平台id
            params.put("reserved_id", reservedId);
            Log.d(TAG, "reservedIdStart: " + reservedId);
            mViewModel.getVolcengineStart(params, new OnNetWorkListener() {
                @Override
                public void onBefore() {
                ...
                public void onSuccess(BaseVo data) {
                    if (data != null && "ok".equals(data.getState())) {
                        ...
                        tv_bar.setText("开启挂机失败,请退出后重试! msg:" + data.getMsg());
                        ll_content_layout.setVisibility(View.VISIBLE);
                        VeGameEngine.getInstance().stop();
                    }
                }
    ...
    *//**
    ...
    public void onError(int errorCode, String errorMessage) {
        ...
        if (10012 <= errorCode && errorCode <= 10016) {
            if (mViewModel != null) {
                ...
                mViewModel.getVolcengineStop(map, new OnBaseCallback<BaseVo>() {
                    ...
                    public void onSuccess(BaseVo data) {
                        if (data != null && data.isStateOK()) {
                            ToastT.warning("云端设备挂机已掉线,请重新开启挂机");
                        }
                    }
                });
            }
    ...
    *//**
    ...
    private GamePodControlService mGamePodControlService;//挂机模式
                    ...
                    if (success) {
                        ...
                        if (tv_hd != null) {
                            switch (hd_type) {
                                case 2:
                                    tv_hd.setText("流畅");
                                    ToastT.success(CloudVeGameHangupActivity.this, "切换至流畅");
                                    break;
                                case 14:
                                    tv_hd.setText("高清");
                                    ToastT.success(CloudVeGameHangupActivity.this, "切换至高清");
                                    break;
                                case 16:
                                    tv_hd.setText("超清");
                                    ToastT.success(CloudVeGameHangupActivity.this, "切换至超清");
                                    break;
                                default:
                                    tv_hd.setText("标清");
                                    ToastT.success(CloudVeGameHangupActivity.this, "切换至标清");
                                    break;
                            }
                        }
                    } else {
                        ToastT.error(CloudVeGameHangupActivity.this, "清晰度切换失败");
                    }
                }
            ...
            mGamePodControlService.setListener(new GamePodControlService.SessionModeListener() {
                *//**
                ...
                public void onResult(int mode, boolean result) {
                    if (mode == 1) {
                        ToastT.success("进入挂机模式");
                    } else {
                        ToastT.success("退出挂机模式");
                    }
    ...
    *//**
    ...
    private void confirmExitGame() {
        CustomDialog tipsDialog = new CustomDialog(this, LayoutInflater.from(this).inflate(R.layout.layout_dialog_cloud_ve_hold_exit, null), WindowManager.LayoutParams.MATCH_PARENT, WindowManager.LayoutParams.WRAP_CONTENT, Gravity.BOTTOM);
    ...
    private void timeOver() {
        CustomDialog tipsDialog = new CustomDialog(this, LayoutInflater.from(this).inflate(R.layout.layout_dialog_cloud_ve_hangup_out, null), WindowManager.LayoutParams.MATCH_PARENT, WindowManager.LayoutParams.WRAP_CONTENT, Gravity.BOTTOM);
        tipsDialog.findViewById(R.id.tv_confirm).setOnClickListener(view -> {
            if (VeGameEngine.getInstance().isPlaying()) {
                VeGameEngine.getInstance().stop();
            }
    ...
    public void onEvent(EventCenter eventCenter) {
        ...
        if (eventCenter.getEventCode() == EventConfig.VE_CLOUD_NO_LOGIN) {
            if (VeGameEngine.getInstance().isPlaying()) {
                VeGameEngine.getInstance().stop();
            }
            finish();
