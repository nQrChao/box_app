...
public class GameNormalItemHolder extends AbsItemHolder<GameInfoVo, GameNormalItemHolder.ViewHolder> {
    ...
    public GameNormalItemHolder(Context context, int leftSidleWidthDP) {
        this(context, false);
        this.leftSidleWidthDP = leftSidleWidthDP;
    }

    public GameNormalItemHolder(Context context, boolean isAddBigPic) {
        super(context);
    ...
    public GameNormalItemHolder(Context context, boolean isAddBigPic, int mBgResource) {
        super(context);
        this.isAddBigPic = isAddBigPic;
        density = ScreenUtil.getScreenDensity(mContext);
        colorList = Arrays.asList(mContext.getResources().getStringArray(R.array.color_list));
    ...
    public void onBindViewHolder(@NonNull ViewHolder viewHolder, @NonNull GameInfoVo gameInfoVo) {
        if (mBgResource != -1){
            viewHolder.mItemView.setBackgroundResource(mBgResource);
        }else {
            viewHolder.mItemView.setBackgroundResource(R.drawable.shape_white_radius_15);
        }
        ...
        if (gameInfoVo.getGame_labels() != null && !gameInfoVo.getGame_labels().isEmpty()) {
            List<GameInfoVo.GameLabelsBean> list = gameInfoVo.getGame_labels().size() > tagSize ?
                    gameInfoVo.getGame_labels().subList(0, tagSize) : gameInfoVo.getGame_labels();
        ...
        viewHolder.rvGameLabels.setLayoutManager(layoutManager);

        // 添加间距装饰器
        int spacing = (int) (1 * ScreenUtil.getScreenDensity(mContext));
        viewHolder.rvGameLabels.addItemDecoration(new HorizontalSpaceItemDecoration(spacing));

        // 设置适配器
        GameNormalLabelAdapter adapter = new GameNormalLabelAdapter(mContext);
        viewHolder.rvGameLabels.setAdapter(adapter);
        adapter.setItems(labelItems);

        // 控制显示逻辑
        if (hasLineTag) {
            viewHolder.rvGameLabels.setVisibility(View.VISIBLE);
            viewHolder.mTvInfoBottom.setVisibility(View.GONE);
        } else {
            viewHolder.rvGameLabels.setVisibility(View.GONE);
                    ...
                    .into(new CustomTarget<Bitmap>() {
                        ...
                        public void onResourceReady(@NonNull Bitmap bitmap, @Nullable Transition<? super Bitmap> transition) {
                            if (bitmap != null && viewHolder.mBigImage.getLayoutParams() != null) {
                                ...
                                int width = ScreenUtil.getScreenWidth(mContext) - ScreenUtil.dp2px(mContext, 20) * 2;
