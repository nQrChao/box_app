...
public class MoreTextView extends LinearLayout {
    ...
    private boolean isExpandViewVisible = true;
    ...
    protected void initialize() {
        ...
        int padding = dip2px(getContext(), 3);
    ...
    protected void bindTextView(int color, float size, final int line, String text) {
        ...
        post(new Runnable() {
            ...
            @Override
            public void run() {
                expandView.setVisibility((contentView.getLineCount() > line) && isExpandViewVisible ? View.VISIBLE : View.GONE);
    ...
    public void expand() {
        ...
        if (mOnExpandListener != null) {
            mOnExpandListener.onExpand(isExpand);
        }
    }
    ...
    public void setArrowVisible(boolean isVisible) {
        isExpandViewVisible = isVisible;
        if (expandView != null) {
            post(() -> {
                expandView.setVisibility(isVisible ? View.VISIBLE : View.GONE);
            });
    ...
    public int getMoreTextLineCount() {
        int lineCount = contentView == null ? 0 : contentView.getLineCount();
        return lineCount;
    }
