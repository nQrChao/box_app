...
public class V2SchemeUtil {
    ...
    public static ApkSectionInfo getApkSectionInfo(File baseApk, boolean lowMemory) throws IOException, SignatureNotFoundException {
        ...
        try {
            ...
            long appAvailableMemory = getAppAvailableMemory();
            if (appAvailableMemory <= ApkSectionInfo.COPY_CONTENT_MAX_SIZE){
                if (appAvailableMemory > 50 * 1024 * 1024){
                    ApkSectionInfo.COPY_CONTENT_MAX_SIZE = 50 * 1024 * 1024;
                }else {
                    ApkSectionInfo.COPY_CONTENT_MAX_SIZE = 0;
                }
            }
             //当apk太大时，复制到ByteBuffer会超过ByteBuffer的可分配空间
